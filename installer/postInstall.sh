...
pacman -S --needed --noconfirm jdk8-openjdk openssh neovim netbeans keepassxc libreoffice-still maven nodejs npm deluge docker virtualbox speedcrunch evolution p7zip tumbler pulseaudio evince vlc qt4 rclone
...
runuser -s /bin/bash -l gbourant -c "git clone https://aur.archlinux.org/expac-git.git ~/expac"
runuser -s /bin/bash -l gbourant -c "git clone https://aur.archlinux.org/aurman.git ~/aurman"


runuser -s /bin/bash -l gbourant -c "cd ~/expac;makepkg -si"
runuser -s /bin/bash -l gbourant -c "cd ~/aurman;makepkg -si"
...
#libcurl-openssl-1.0
#gitg
runuser -s /bin/bash -l gbourant -c "aurman -S --noconfirm --noedit google-chrome gscreenshot visual-studio-code-bin skypeforlinux-stable-bin gpicview-gtk3 git-cola"
runuser -s /bin/bash -l gbourant -c "aurman -S --noconfirm --noedit paper-gtk-theme-git paper-icon-theme-git"
